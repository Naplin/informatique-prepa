...
g1_lst = [[1, 3], [0, 2, 3, 4], [1, 4], [0, 1], [1, 2]]
...
def matrice_vers_liste(mat_g):
    n = len(mat_g)
    lst_g = [[] for _ in range(n)]
    for i in range(n):
        for j in range(n):
            if mat_g[i][j]:
                lst_g[i].append(j)
...
def liste_vers_matrice(lst_g):
    n = len(lst_g)
    mat_g = [[False] * n for _ in range(n)]
    for i in range(n):
        for j in lst_g[i]:
            mat_g[i][j] = True
    return mat_g
...
assert matrice_vers_liste(g1_mat) == g1_lst
assert liste_vers_matrice(g1_lst) == g1_mat
assert matrice_vers_liste(g2_mat) == g2_lst
assert liste_vers_matrice(g2_lst) == g2_mat


def degre_NO(lst_g, s):
    return len(lst_g[s])


assert degre_NO(g1_lst, 0) == 2
assert degre_NO(g1_lst, 1) == 4


def degre_O_entrant(lst_g, s):
    n = len(lst_g)
    deg = 0
    for i in range(n):
        if s in lst_g[i]:
...
assert degre_O_entrant(g2_lst, 0) == 0
assert degre_O_entrant(g2_lst, 1) == 2


def degre_NO_mat(mat_g, s):
    """Renvoie le degré sortant du sommet s
    dans le graphe mat_g"""
    n = len(mat_g)
    deg = 0
    for j in range(n):
        if mat_g[s][j]:
...
def degre_O_mat(mat_g, s):
    """Renvoie le degré entrant du sommet s
    dans le graphe mat_g"""
    n = len(mat_g)
    deg = 0
    for i in range(n):
...
assert degre_NO_mat(g1_mat, 0) == 2
assert degre_NO_mat(g1_mat, 1) == 4
assert degre_O_mat(g2_mat, 0) == 0
assert degre_O_mat(g2_mat, 1) == 2
...
assert est_connexe(g1_lst)
assert not est_connexe([[1], [0], [3], [2], [3]])
...
labyrinthe_lst = matrice_vers_liste(labyrinthe_lst)
